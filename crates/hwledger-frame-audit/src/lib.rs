...
pub const PLACEHOLDER_KEYWORDS: &[&str] =
    &["placeholder", "run-journeys", "pending on user"];
...
pub fn detect(path: &Path) -> Result<Detection> {
    ...
    Ok(Detection { path: path.to_path_buf(), width, height, word_count, coverage, ocr_text, flagged })
}
...
pub fn render_audit_markdown(flagged: &[FlaggedFrame], total_scanned: usize) -> String {
    ...
    md.push_str(
        "Generated by `hwledger-frame-audit`. Flags GUI-journey keyframes whose OCR text \
         coverage > 0.18 AND word count > 30 — the empirical signature of the \
         `generate-placeholder-artefacts.py` gradient cards that let the VLM/OCR judge \
         cheat by reading the placeholder's descriptive text back as evidence.\n\n",
    );
