...
import { meanQualityPass, summaryQualityPass, summaryQualityLabel } from '../lib/metrics';
    ...
    <div className="view-content">
      ...
      <div className="suite-grid" id="suiteGrid">
        {[...suites.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([suite, grp]) => {
          ...
          return (
            <div key={suite} className="suite-card" role="button" tabIndex={0}
              onClick={() => onJumpToSuite(suite)}
              onKeyDown={(e) => { if (e.key === 'Enter') onJumpToSuite(suite); }}>
