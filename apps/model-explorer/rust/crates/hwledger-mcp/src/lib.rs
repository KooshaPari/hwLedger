...
//! * [`tools`]           — six stub tool implementations + their schemas.
//! * `McpServer` / `McpState` (this module) — request validation + dispatch.
...
///
/// For v1 the only mutable bits are the MCP `initialized` flag and the
/// `client_info` captured from the `initialize` handshake. Real backends
/// (search index, RAG store, embedder) will be added here in later phases.
...
impl McpState {
    /// Construct a fresh, uninitialized state.
    pub fn new() -> Self {
        Self::default()
    }
}
...
impl McpServer {
    ...
    pub fn handle_tools_call(&self, params: Option<&Value>) -> Result<Value, McpError> {
        let params = params.unwrap_or(&Value::Null);
        let obj = params
            .as_object()
        ...
        let value = tools::call_tool(name, &args)?;
