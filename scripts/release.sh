...
echo "Notary Key ID:  $APPLE_NOTARY_KEY_ID"
echo
...
APP_PATH="$REPO_ROOT/apps/macos/build/HwLedger.app"
if [[ ! -d "$APP_PATH" ]]; then
...
SPARKLE_PRIVATE_KEY_PATH="$SPARKLE_PRIVATE_KEY_PATH" \
  ./scripts/generate-appcast.sh "$REPO_ROOT/apps/macos/build"

# 6. Copy appcast into docsite.
echo "[6/6] Publishing appcast to docsite"
cp "$REPO_ROOT/apps/macos/build/appcast.xml" "$REPO_ROOT/docs-site/public/appcast.xml"
